...
public class Pj3dCone extends Pj3dToolbox
{
	private Pj3d parent;
	...
	///
	public Pj3dCone(Pj3d parent, float radius, float height)
	{
		this.parent = parent;
	...
	/// set the Box to the "zero" point of the world
	///
	public Pj3dCone(Pj3d parent, float radius, float height, int xdivisions, int ydivisions)
	{
		this.parent = parent;
		this.transform = new Pj3dTransform();
		this.shader = new Pj3dShader(parent);
	...
	private void InitPrimitive(float radius, float height, int xdivisions, int ydivisions)
	{
		...
		Cone cone = null;
		if (xdivisions == -1)
			cone = new Cone(radius, height, primflags, shader.appearance);
		else
			cone = new Cone(radius, height, primflags, xdivisions, ydivisions, shader.appearance);
		
		cone.setCapability(Cone.ALLOW_CHILDREN_READ);
		
		for (int i=0; i<cone.numChildren(); i++)
		{
			cone.getShape(i).setCapability(Shape3D.ALLOW_GEOMETRY_READ);
			cone.getShape(i).setCapability(Shape3D.ALLOW_APPEARANCE_READ);
			cone.getShape(i).setCapability(Shape3D.ENABLE_PICK_REPORTING);
		}
		
		transform.transformgroup.addChild( cone );
